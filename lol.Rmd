...
**Conclusion:** Looking at the top 10 factors for winning, we see that the most important factors are Gold (Blue Total Gold + Blue Gold Per Minute), Experience (Blue Total Experience + Blue Average Level - a factor directly affected by Experience as you level up while gaining Experience),  Kills (Blue Kills and Blue Assists - a factor affected by kills if you help your teammates take down a champion) and Creep Score (CS) (Blue Elite Monster Killed + Blue Total Minions Killed + Blue CS Per Minute). Note that Elite Monster and Minions count as creeps.
...
Our assumption is as followed: A team should play aggressively and aim to get as much kill as possible and translate it into further advantages mentioned above. We define a successful aggressive as a match where the blue team gets 1 kill per minute or more. While playing aggressively, we will undoubtedly make mistakes, so to account for that, we will let the red team to have 5 kills or more. Note that here, we are exploring only blue side because there is not much of a difference between Blue and Red in ranked matches - the game is 99% the same playing either side.
...
#### Understanding how successfully playing aggressive affect number of elite monsters taken
...
#### Conclusions
League of Legends is a very interesting game: There are multiple approaches one could take in the game, and there exists no concrete formula to guarantee a win. However, in ranked games, we can capitalize on that to boost our winning chance by playing aggressive in the early game. Successfully playing aggressive in the early game translates into winning the match quite well. This argument is supported by the EDA above.

So, we know that we should go for strong early game tactics. There are many ways to do it, but in ranked game, champions have the most influence in the outcome of the match. the As such, we should pick champions who are extremely strong in the early game that could go for kills. They could be lane bullies, assassins, gankers, or roamers. The motivation behind the increase in probability of winning when we successfully play aggressive could be due to 
